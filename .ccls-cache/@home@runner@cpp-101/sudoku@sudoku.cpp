...
bool Sudoku::validMove(int val, int row, int col) const {
  ...
  if (std::find(this->board.data[row].begin(), this->board.data[row].end(),
                val) != this->board.data[row].end()) {
    return false;
  }
  ...
  for (int m = (std::floor(row / 3)) * 3; m < (std::floor(row / 3)) * 3 + 3;
       m++) {
    for (int n = (std::floor(col / 3) * 3); n < (std::floor(col / 3)) * 3 + 3;
         n++) {
...
grid Sudoku::getBoard() const { return this->board; }
...
bool Sudoku::validSquares() const {
  for (int i = 3; i < 10; i += 3) {
    for (int j = 3; j < 10; j += 3) {
      ...
      for (int k = 0; k < 3; ++k) {
        square.insert(square.end(),
                      this->board.data[i - 3 + k].begin() + (j - 3),
